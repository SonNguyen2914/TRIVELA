# Runbook — first MLS shadow slate (`mls-shadow-v1.4`)

Operational protocol for the first prospective MLS T-10 slate under the
V9.3 baseline. Record evidence at each stage; do not merely observe
...
```text
artifact_schema            = model-input-v5      (was v3 at v1.2)
snapshot policy_version    = mls-lock-v2         (was mls-lock-v1)
...
- `GET /api/mls/paper` — signals, fills, rejects with reasons. **Zero fills
  with all rejects `NOT_EXECUTION_READY` is the v1 failure mode.**
