"""liga-mx-2026-v0 — the Liga MX model scaffold (2026-07-29). SHIPS DARK.
...
fitted from Liga MX data held in the live database. Three deliberate
notes:

  1. GOALS-ONLY. No trustworthy xG source exists for Liga MX in this
     stack (research_archive/ligamx_official_api_probe_2026-07-29.json:
     ESPN mex.1 carries xG only as per-leader crumbs — the single
     leading player's expectedGoals per category — never team-level
     match xG; the official ligamx.net surface is an obfuscated
     internal RPC with no public contract, rejected; scraping sources
     are rejected). There is no Liga MX analogue of MLS_XG_RATING_ALPHA
     and no xG anywhere in this module — the gap is documented, not
     papered over.

  2. DARK BY CONSTRUCTION. No approval decision exists for liga-mx-2026,
     ensure_model_version is only ever called with
     approved_for_shadow=False, and no code path here or anywhere else
     can create a Liga MX approval. The F3/F9 gates in src/live/runs.py
     therefore refuse every Liga MX run and every lock, and no Liga MX
     odds render on any surface. Approval must be EARNED through the
     evaluation ladder — which also means the shrinkage/dispersion/
     calibration constants below are STARTING POINTS carried from the
     closest measured precedent (MLS league play), not validated
     Liga MX values. Re-sweep on Liga MX data before any approval.

  3. SPLIT SEASONS. The liga-mx-2026 competition spans the full ESPN
     season year (Apertura 2026 + Clausura 2027 + both Liguillas —
     research_archive/ligamx_RESEARCH_SUMMARY_2026-07-29.json,
     competition_identity_decision). fit() therefore sees completed
     fixtures across the tournament boundary, discounted only by the
     recency half-life. That is a MODELLING CHOICE, stated here rather
     than hidden: whether Clausura ratings should reset, reweight or
     carry Apertura history is an open question the approval evaluation
     must answer on real data before this model ever runs. Within the
     current Apertura only Apertura fixtures exist, so the question has
     no effect on anything this build does.

Season context: 18 clubs, 17 rounds per tournament + Liguilla; Apertura
2026 is IN SEASON (2 matchdays completed at build time). Until enough
...
MODEL_NAME = "liga-mx-2026-v0"
COMPETITION_SLUG = "liga-mx-2026"

# STARTING POINTS, not Liga MX-validated (see module docstring).
# SHRINK_GAMES and HALF_LIFE_DAYS were swept on MLS 2026 (162 fixtures);
# MIN_GAMES is the shared history floor. Re-sweep before any approval.
...
def calibrate(outcomes: dict, alpha: float) -> dict:
    """Shrink the simulated 3-way toward uniform. Liga MX default alpha
    is 0.0 (no Liga MX overconfidence has been MEASURED yet — the MLS
    0.25 was measured on MLS data and does not transfer by assumption)."""
    if alpha <= 0:
        return outcomes
    a = min(alpha, 1.0)
...
def seed_for(fixture, run_type: str) -> int:
    ...
    identity, masked to 31 bits (signed int4 on PostgreSQL). The
    MODEL_NAME:COMPETITION_SLUG prefix gives Liga MX its own seed
    domain — the same espn_event_id seeds differently here than under
    model_mls or model_epl."""
...
INPUT_ARTIFACT_SCHEMA = "model-input-ligamx-v1"
_GIT_REV = os.getenv("RAILWAY_GIT_COMMIT_SHA", "")[:40]


def _canonical(doc: dict) -> str:
    """Canonical JSON with ensure_ascii=False — Liga MX names carry
    Spanish accents (América, Querétaro), and an ensure_ascii default
    once produced false corpus mismatches in this project. The bytes
    hashed are the UTF-8 of the real characters, not \\u escapes."""
...
def engine_signature(code_revision: str | None = None) -> dict:
    """Fingerprint of the ACTUAL engine implementation + runtime a
    Liga MX replay depends on — same design as model_mls.engine_signature
    (V9.1 eval F5), hashing THIS module's source in place of model_mls's."""
    import importlib
    import platform
    ...
    constants = {
        ...
        "goal_dispersion_cv": config.LIGAMX_GOAL_DISPERSION_CV,
    ...
    for name in ("src.live.model_ligamx", "src.models.simulator",
...
def build_input_artifact(fixture, model: dict,
    ...
    because no xG exists (schema model-input-ligamx-v1)."""
    ...
    doc = {
        ...
        "calibration": {
            "alpha": config.LIGAMX_CALIBRATION_ALPHA,
            "anchor": "uniform_3way",
        },
        "simulation": {
...
def predict_fixture(fixture, model: dict, run_type: str = "scheduled",
    ...
    sim = MatchSimulator(n_simulations=n_sims,
                         seed=seed_for(fixture, run_type),
                         dispersion_cv=config.LIGAMX_GOAL_DISPERSION_CV)
    out = sim.simulate(home, away, stage="group")
    outcomes = calibrate(out["outcomes"], config.LIGAMX_CALIBRATION_ALPHA)
...
def current_model() -> dict | None:
    """Fit from everything completed as of now. None until enough
    completed fixtures exist — the honest empty."""
    if not plane_ready():
        return None
...
def backtest(n_sims: int = 4000) -> dict:
    """Walk-forward over the ingested history vs the flat-ratings
    baseline — exactly model_mls.backtest's design. Nearly empty until
    the Apertura accumulates completed fixtures; approval evaluation
    builds on this."""
    if not plane_ready():
...
def ensure_model_version(approved_for_shadow: bool = False) -> None:
    """Upsert the model_version row. The Liga MX boot only ever calls
    this with approved_for_shadow=False — the model is DARK, and
    approval is an explicit operator act after an evaluation that does
    not exist yet. approved_for_real_money is NEVER set anywhere."""
    if not plane_ready():
        return
    from datetime import datetime
    s = get_session()
    try:
        ...
        if row is None:
            row = ModelVersion(name=MODEL_NAME,
                               description="Liga MX goals-rate baseline "
                                           "scaffold (DARK: unapproved, "
                                           "no xG source exists; spans "
                                           "Apertura+Clausura — see "
                                           "module docstring)",
