...
def test_engine_signature_modules_are_byte_identical_to_base():
    ...
    This assertion message is the ONE place the rule can be stated at the
    moment someone breaks it. It cannot be a header comment inside the
    four modules — writing "do not edit this file" into them would be the
    edit, and would move the very hash the comment warns about (measured:
    one prepended comment line moved xg_model's contribution and the
    signature with it). So the message carries the whole explanation
    rather than a summary, and `src/models/ENGINE-SIGNATURE.md` sits
    beside the files for anyone reading rather than failing.
    """
    from src.live import model_mls
    got = model_mls.engine_signature()["source_sha256"]
    changed = sorted(k for k, v in got.items()
                     if ENGINE_SOURCE_SHA256.get(k) != v)
    assert got == ENGINE_SOURCE_SHA256, (
        f"\n\nENGINE-SIGNATURE MODULE CHANGED: {', '.join(changed)}\n"
        "\n"
        "engine_signature() sha256s the RAW BYTES of these four modules, "
        "so a helper, a docstring, a type hint or a bare COMMENT moves the "
        "hash exactly as a changed simulator would. A moved hash fails BOTH "
        "arms of the boot approval check, so the plane goes DARK and stays "
        "dark until an operator calls /api/admin/mls/approval/activate. "
        "Shadow evidence collection stops meanwhile.\n"
        "\n"
        "The guard observes bytes, not intent, and must not try to tell "
        "them apart — a signature that exempted 'small' edits would exempt "
        "whatever each author considered small.\n"
        "\n"
        "IF THIS WAS NOT A DELIBERATE MODELLING CHANGE: move it to a "
        "sibling module and pin the two equal by test. src/live/revision.py "
        "is a worked example — it exists apart from model_mls ONLY for this "
        "reason.\n"
        "\n"
        "IF IT WAS: that is the mechanism working. Sequence it between "
        "slates, have the operator reactivate, and measure.\n"
        "\n"
        "EITHER WAY: DO NOT PASTE THE NEW HASH IN HERE. These literals are "
        "a guard, not a snapshot. See src/models/ENGINE-SIGNATURE.md and "
        "AGENTS.md section 4.\n")
